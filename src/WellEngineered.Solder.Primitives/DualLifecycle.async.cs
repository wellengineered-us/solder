...
namespace WellEngineered.Solder.Primitives
{
		...
		: IAsyncLifecycle
	{
		...
		protected async virtual ValueTask MaybeInitializeAsync(CancellationToken cancellationToken = default)
		{
			await this.ExplicitInitializeAsync(cancellationToken);
		}

		protected async virtual ValueTask MaybeSetIsAsyncCreatedAsync(CancellationToken cancellationToken = default)
		{
			await this.ExplicitSetIsAsyncCreatedAsync(cancellationToken);
		}

		protected async virtual ValueTask MaybeSetIsAsyncDisposedAsync(CancellationToken cancellationToken = default)
		{
			await this.ExplicitSetIsAsyncDisposedAsync(cancellationToken);
		}

		protected async virtual ValueTask MaybeTerminateAsync(CancellationToken cancellationToken = default)
		{
